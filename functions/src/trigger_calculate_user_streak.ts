...
exports.handler = async function (snap, db) {
    ...
    const snapshot = await db.collection(COLLECTIONS.TASKS).where('isDone', '==', true).orderBy('createdAt', 'desc').get();
    ...
    db.collection(COLLECTIONS.USERS).doc(createdTask.user.id).update({ streak: streak });
};
